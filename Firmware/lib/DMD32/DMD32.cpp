...
DMD::DMD(byte panelsWide, byte panelsHigh) {
  ...
  spi->begin(PIN_DMD_CLK, -1, PIN_DMD_R_DATA, -1);
...
void DMD::scanDisplayBySPI() {
  ...
  if (digitalRead(PIN_OTHER_SPI_nCS) == HIGH) {
    ...
    for (int i = 0; i < rowsize; i++) {
      data[i * 4] = bDMDScreenRAM[offset + i + row3];
      data[i * 4 + 1] = bDMDScreenRAM[offset + i + row2];
      data[i * 4 + 2] = bDMDScreenRAM[offset + i + row1];
      data[i * 4 + 3] = bDMDScreenRAM[offset + i];
    }
    spi->writeBytes(data, rowsize * 4);
